...
ScopeBullet::ScopeBullet()
	: BaseBullet()
{
	m_BulletType = ShootType::Scope;
}

ScopeBullet::ScopeBullet(XMFLOAT3 position, XMFLOAT3 rotation, XMFLOAT3 scale,
...
bool ScopeBullet::Initialize(ID3D11Device* device)
{
...
{
	SetPosition(PlayerPosition.x, PlayerPosition.y + 0.30f, PlayerPosition.z);
	...
	transform.eulerRotation.x -= XM_PI * 0.03f;
...
void ScopeBullet::Execute(XMFLOAT3 pos)//bullet update
{
	// 중력 + 장약으로 인한 변위
	XMVECTOR tv = { 0.0f, -0.000098f * timer, speed * GetDeltaTime() * 0.45f, 0 };
	...
	// 라디안 값 변환 및 회전 계산
	transform.rotation.x -= pitchAngle * 0.0174532925f;
