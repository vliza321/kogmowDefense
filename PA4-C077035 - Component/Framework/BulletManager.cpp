...
bool BulletManager::Initialize(ID3D11Device* device)
{
	bool result = true;
	result = InitializeBulletMap();
	m_AllActivatedBullets.clear();
	m_ReloadTimer = 0;
	m_DoReload = false;
	m_CurrentShootType = ShootType::FPC;
	...
	result = InitializeAllBullet(device);
	return result;
}

bool BulletManager::InitializeBulletMap()
{
	bool result = true;
...
bool BulletManager::InitializeAllBullet(ID3D11Device* device)
{
	bool result = true;
	for (auto& nb : *(m_BulletMapSet[ShootType::FPC].get()))
	{
		nb->Initialize(device);
	}
	...
	{
		ab->Initialize(device);
	}
	return result;
}

void BulletManager::Execute(XMFLOAT3 PlayerPos )
{
	for (auto& nb : m_AllActivatedBullets)
	{
		nb->Execute(PlayerPos);
	}
...
void BulletManager::LateExecute(XMFLOAT3 PlayerPos)
{
...
void BulletManager::FixedExecute(XMFLOAT3 PlayerPos)
{
	for (auto& nb : m_AllActivatedBullets)
	{
		nb->FixedExecute(PlayerPos);
	}
}