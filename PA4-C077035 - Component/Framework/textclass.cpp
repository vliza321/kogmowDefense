...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, 
	int screenWidth, int screenHeight, XMMATRIX baseViewMatrix, int allObjectsCount, int allPolygonCount)
{
	bool result;
	...
	string str = "Hello";//"AllObject : " + std::to_string(allObjectsCount) + "AllPolygon : " + std::to_string(allPolygonCount);
	s = str.c_str();
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence1, "asdfasdf", 200, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "ohwonkf", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
