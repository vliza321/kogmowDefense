...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext,  int indexCount, int instanceCount, XMMATRIX rotationMatrix, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
								XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	result = SetShaderParameters(deviceContext, rotationMatrix, worldMatrix, viewMatrix, projectionMatrix, texture);
	if(!result)
	{
		return false;
	}
	...
	const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	samplerDesc[0].Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
	...
	samplerDesc[2].Filter = D3D11_FILTER_ANISOTROPIC;
	...
	samplerDesc[2].MaxAnisotropy = 16;
...
bool TextureShaderClass::ChangeFilterMode(ID3D11Device* device, int a)
{
	bool result;
	result = device->CreateSamplerState(&samplerDesc[a], &m_sampleState);
	if (FAILED(result))
	{
		return false;
	}
...
bool TextureShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX rotationMatrix, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
											 XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
